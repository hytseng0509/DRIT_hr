...
class MultiScaleDis(nn.Module):
  def __init__(self, input_dim, n_scale=2, n_layer=4, norm='None', sn=False):
  ...
  def forward(self, x, x_s=None):
    ...
    outs = [self.Diss[0](x), self.Diss[1](x_s)]
    '''outs = []
...
class G(nn.Module):
  def __init__(self, output_dim_a, output_dim_b, nz):
    ...
    decA6 += [nn.Tanh()]
    self.decA5 = decA5
    self.decA5_1 = nn.Sequential(*decA5_1)
    self.decA6 = nn.Sequential(*decA6)
    ...
    decB5_1 = [nn.ConvTranspose2d(tch//2, output_dim_a, kernel_size=1, stride=1, padding=0), nn.Tanh()]
    tch = tch//2
    decB6 = []
    decB6 += [INSResBlock(tch, tch), INSResBlock(tch, tch), ReLUINSConvTranspose2d(tch, tch//2, kernel_size=3, stride=2, padding=1, output_padding=1)]
    tch = tch//2
    decB6 += [nn.ConvTranspose2d(tch, output_dim_b, kernel_size=1, stride=1, padding=0)]
    decB6 += [nn.Tanh()]
    self.decB5 = decB5
  ...
  def forward_a(self, x, z):
    ...
    out5_img = self.decA5_1(out5)
    out6 = self.decA6(out5)
  ...
  def forward_b(self, x, z):
    ...
    out5 = self.decB5(out4)
    out5_img = self.decB5_1(out5)
    out6 = self.decB6(out5)
    return out6, out5_img
...
class G_concat(nn.Module):
  def __init__(self, output_dim_a, output_dim_b, nz):
    ...
    self.decA2_1 = nn.Sequential(*decA2_1)
    ...
    self.decB2_1 = nn.Sequential(*decB2_1)
...
def conv3x3(in_planes, out_planes, bias=True):
  return [nn.ReflectionPad2d(1), nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=1, padding=0, bias=bias)]
